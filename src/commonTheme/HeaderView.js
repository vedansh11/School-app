...
export const DashboardHeaderView = (props) => {
    ...
    return (
        <View style={stylesCommon.mainBackground}>
            <TouchableOpacity style={stylesCommon.homeView}
                onPress={() => HomeNavigation()}>
                <Image source={icon.IC_HOME}
                    style={stylesCommon.homeicon}></Image>
            </TouchableOpacity>
            ...
            <View style={stylesCommon.notificationView}>
                ...
                <View>
                    <TouchableOpacity style={[stylesCommon.notificationIcon,{marginEnd:15}]}
                        onPress={() => NotificationClick()}>
                        <Image source={icon.IC_NOTIFICATION}
                ...
                <View>
                    <TouchableOpacity style={stylesCommon.notificationIcon}
                        onPress={() => Logout()}>
                        <Image source={icon.IC_LOGOUT}
                            style={[stylesCommon.notificationIcon,{tintColor:'#ffffff'}]}
                            >
                        </Image>
   ...
   // console.log(props);
    ...
    useEffect(() => {
        if ((props.type === 'teacher')) {
            Preference.GetData(PreferenceKeys.TEACHER_SCHOOL_DETAIL)
                .then(function (value) { 
                    setSchoolInfo(JSON.parse(value)) 
                })
        } else if (props.type === 'parent') {
            Preference.GetData(PreferenceKeys.STUDENT_DETAIL)
                .then(function (value) {
    ...
    return (
        <View style={(props.screen != 'TeacherSupport' 
        &&props.screen != 'ParentSupport' 
        && props.screen != 'Payment'
       // && props.screen != 'DairyView'
        )? stylesCommon.mainBackground : stylesCommon.mainBackground_custome }>
            <TouchableOpacity style={stylesCommon.homeView}
                onPress={() => HomeNavigation()}>
                <Image source={icon.IC_BACK_ARROW}
                    style={stylesCommon.homeicon}></Image>
            </TouchableOpacity>
            {
           ...
           
            {
                (props.screen != 'Notification' ) ?
                    <View style={(props.screen != 'TeacherSupport' 
                    &&props.screen != 'ParentSupport' 
                    && props.screen != 'Payment') ? stylesCommon.notificationView : stylesCommon.notificationView_new}>
                        {

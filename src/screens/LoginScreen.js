...
const LoginScreen = ({ navigation }) => {
    ...
    function VerifyMobilNumberAPI(clickType) {
        ...
        axiosCallAPI('post', Utills.VERIFY_MOBILE_NUMBER, loginFormData, requestOptions, true, navigation)
            .then((response) => {
    ...
    function verifyOTP_API() {
        ...
        axiosCallAPI('post', Utills.VERIFY_OTP, loginFormData, requestOptions, true, navigation)
            .then(response => {
                if (response !== undefined) {
                    ...
                    if (response.status) {
                        ...
                        Preference.SetData(PreferenceKeys.TOKEN, 'Bearer ' + response.data.access_token)
                        verifyUserProfile()
                    }
                    else {
    ...
    async function verifyUserProfile() {
        ...
        axiosCallAPI('get', Utills.VERIFY_PROFILE, '', requestOptions, true, navigation)
            .then(response => {
                setLoaderView(false)
                console.log("_________________________________")
                console.log(response);
                Preference.SetData(PreferenceKeys.CURRENT_USERID, response.id);
                Preference.SetData(PreferenceKeys.LOGIN_USER_DETAIL, JSON.stringify(response));
                const Roles = Object.values(response.roles).map((value) => value.slug)

                console.log('**  Roles ** ' + Roles);
                if (Roles.includes('parents') && Roles.includes('guardian') && Roles.includes('teacher')) {
                    console.log('Both');
            ...
            <View style={stylesCommon.MainContainer}>
                ...
                <View style={stylesCommon.whiteCornerStyle}>
                    {/* <View style={stylesCommon.parentViewStyle}>
                        <Text style={stylesCommon.continueTextStyle}> {AppText.WELCOM_TEXT} </Text>
                    </View> */}
                    ...
                    <OutlinedTextField
                        ...
                        maxLength={10}
                        autoFocus={true}
                        error={mobileNumber.length === 0 ? AppText.ENTER_NUMBER :
                    ...
                    <View style={{ flexDirection: 'row', marginTop: 10, justifyContent: 'space-between', alignItems: 'center' }}>
                        {
                            (isShowButton) && <Text style={stylesCommon.otpTextStyle}> {AppText.OTP} </Text>
                        }
                        {
                            showTimer ?
                                <CountDown
                                    ...
                                    digitTxtStyle={{ color: color.INFO_BLUE, }}
                                    separatorStyle={{ color: color.INFO_BLUE, }}
                                    timeToShow={['M', 'S']}
                                    timeLabels={{ m: '', s: '' }}
                                    showSeparator
                                /> : null
...

                        {
                            showResend ?
                                !showTimer ?
                                    <TouchableOpacity style={stylesCommon.otpTextGreenStyle}
                                        onPress={() => VerifyMobileNumber('resendOTP')}>
                                        <Text style={stylesCommon.otpTextGreenStyle}> {AppText.RESEND_OTP} </Text>
                                    </TouchableOpacity> : null
                                : <TouchableOpacity style={stylesCommon.otpTextRedStyle}
                                    onPress={() => VerifyMobileNumber('OTP')}>
                                    <Text style={stylesCommon.otpTextRedStyle}> {AppText.GET_OTP} </Text>
                                </TouchableOpacity>
                        }
                        ...
                        (isShowButton) ?
                            <View>
                                <OTPTextView
                                    ...
                                    autoFocus={false}
                                    returnKeyType='done'
                                    selectionColor={color.APP_SECONDARY}
                                ...
                                <View style={styles.checkboxContainer}>
                                    <CheckBox
                                        ...
                                        tintColors={{ true: color.APP_SECONDARY }}
                                        onCheckColor={color.WHITE}
                                        onFillColor={color.APP_SECONDARY}
                                        onTintColor={color.APP_SECONDARY}
                                        boxType={'square'}
                                    ...
                                    <View style={{ flexDirection: 'column', }}>
                                        <View style={{ flexDirection: 'row', }}>
                                            ...
                                            <TouchableOpacity style={styles.labelUnderline}>
                                                <Text style={styles.labelUnderline}>{'Terms of Services'}</Text>
                                            </TouchableOpacity>
                                            <Text style={styles.label}>{'and'}</Text>
                                        </View>
                                        <TouchableOpacity style={styles.labelUnderline}>
                                            <Text style={styles.labelUnderline}>{'Privacy Policy'}</Text>
                                        </TouchableOpacity>
                                    </View>

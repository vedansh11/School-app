...
import { Alert, Modal, StyleSheet, Text, FlatList, Pressable, StatusBar, View, SafeAreaView, ScrollView, Image, TextInput, TouchableOpacity, Platform } from "react-native";
...
import { DashboardRawDetailMenu, renderEmptyContainer, TitileBackgroundView, ModelTitleView, ButtonView } from '../../commonTheme/CommonView';
...
export const DairyView = (props) => {
    ...
    const [effectiveDateTEXT, setEffectiveDateTEXT] = useState(AppText.EFFECTIVE_DATE)
    ...
    useEffect(() => {
        ...
        effectiveFieldRef?.current?.setValue(moment(effectiveDate).format('DD/MM/YYYY'))
    ...
    async function NoteTypeListAPI() {
        ...
        axiosCallAPI('get', Utills.NOTE_TYPE_LIST, '', requestOptions, true, props.navigation)
            .then(response => {
    ...
    async function DeleteDairyAPI(dairyDeleteID) {
        ...
        axiosCallAPI('get', Utills.TEACHER_DELETE_DIARY + '?id=' + dairyDeleteID, '', requestOptions, true, props.navigation)
            .then(response => {
    ...
    async function TeacherDiaryAPI(selectedDate) {
        ...
        if (selectedDate != '')
            var dairyListURL = Utills.TEACHER_DIARY_LIST + '?sectionId=' + props.sectionId + '&search=' + moment(selectedDate).format('YYYY-MM-DD')
        else
            var dairyListURL = Utills.TEACHER_DIARY_LIST + '?sectionId=' + props.sectionId

            console.log('dairyListURL >>'+dairyListURL);
        axiosCallAPI('get', dairyListURL, '', requestOptions, true, props.navigation)
            .then(response => {
    ...
    function validation() {
         if ((noteTypeSelected === '' && noteTypeID === '') && subjectSelectedName.length === 0) {
    ...
    const EditDairyClick = (item) => {
        ...
        effectiveFieldRef?.current?.setValue(moment(new Date(item.effectiveDate)).format('DD/MM/YYYY'))
        dueFieldRef?.current?.setValue(moment(new Date(item.dueDate)).format('DD/MM/YYYY'))
    ...
    async function AddDiaryAPI() {
        ...
        loginFormData.append("effectiveDate", moment(effectiveDate).format('YYYY-MM-DD'))
        ...
        axiosCallAPI('post', Utills.TEACHER_SAVE_DIARY, loginFormData, requestOptions, true, props.navigation)
            .then((response) => {
                if (response !== undefined) {
                    setModalVisible(!modalVisible)
                    clearData()
            ...
            <View>
                {
                    ...
                    <View style={stylesCommon.inputMainView}>
                        <SelectDropdown
                            ...
                            defaultButtonText={(noteTypeSelected === '') ? 'Select note type' : noteTypeSelected}
                        ...
                        <DatePicker
                            ...
                            onConfirm={(date) => {
                                ...
                                if(effectiveDate <  date){
                                    ...
                                    Alert.alert( AppText.ALERT_APP_NAME,"Due date should be greater than the effective date")
                                }
                            }}
                            onCancel={() => {
                                setDueDateOpen(false)
                    ...
                    <View style={styles.centeredView}>
                        <ScrollView keyboardShouldPersistTaps='handled'
                            ...
                            <View style={styles.modalView}>
                                <ModelTitleView
                                    tiitle={(dairyID === 0) ? AppText.ADD_DIARY : AppText.EDIT_DIARY}
                                    onPressClose={() => renderCloseClick()} />
                                <View style={{
                                    marginBottom: 20,
                                    width: '100%'
                                }}>
                                    {InputView(AppText.NOTE_TYPE, false, icon.IC_DOWN_ARROW, false)}
                                    {InputView(AppText.EFFECTIVE_DATE, false, icon.IC_CALENDAR, false)}
            ...
            <View>
                {
                    (props.dairyType == 'teacher') ?
                        (selectedTopIndex == mainIndex && selectedIndex == index) ?
                            <Tooltip
                                ...
                                content={
                                    <View style={stylesCommon.tooltipContent}>
                                        <TouchableOpacity
                                            onPress={() => EditDairyClick(item)}>
                                            <Text style={stylesCommon.tooltipText}>{AppText.EDIT}</Text>
                                        </TouchableOpacity>
                                        <View style={stylesCommon.tooltipSepretor} />
                                        <TouchableOpacity
                                            onPress={() => DeleteDairyClick(item.id)}>
                                            <Text style={stylesCommon.tooltipText}>{AppText.DELETE}</Text>
                                        </TouchableOpacity>
                                    </View>
                                }
                                placement="top"
                            ...
                            >
                                <TouchableOpacity style={[styles.diaryRowView, { width: '100%' }]}
                                    onPress={() => handleClick(index, mainIndex)}>
                                    <Text style={stylesCommon.diaryRowTitle}>{item.subjectName}</Text>
                                    <Text style={stylesCommon.diaryRowDescription}>{item.description}</Text>
                                    <Text style={stylesCommon.diaryTimeView}>{item.dueDate}</Text>
                                </TouchableOpacity>
                            </Tooltip>
                            :
                            <TouchableOpacity style={styles.diaryRowView}
                                onPress={() => handleClick(index, mainIndex)}>
                                <Text style={stylesCommon.diaryRowTitle}>{item.subjectName}</Text>
                                <Text style={stylesCommon.diaryRowDescription}>{item.description}</Text>
                                <Text style={stylesCommon.diaryTimeView}>{item.dueDate}</Text>
                ...
                <TitileBackgroundView
                    ...
                    onSecondViewClick={() => {
                        if (props.dairyType === 'teacher') {
                            ...
                            setDairyID(0)
                            setModalVisible(true);
                            effectiveFieldRef?.current?.setValue(moment(new Date()).format('DD/MM/YYYY'))
                            dueFieldRef?.current?.setValue(moment(new Date()).format('DD/MM/YYYY'))
                        }
                    }}
                    onClick={() => {
                        TeacherDiaryAPI('')          
                    ...
                    <View
                        ...
                        <TouchableOpacity 
                         style={{
                            ...
                            setFilterDate(true)
                            fieldRef?.current?.setValue(moment(new Date()).format('DD/MM/YYYY'))
                            }}>
                        <Text style={{
